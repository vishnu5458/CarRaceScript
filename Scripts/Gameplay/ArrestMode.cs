...
public class ArrestMode : GamePlaySource
{
	[Space(5)]
	...
	protected override void Update ()
	{
		...
		{
			if(activeBots[i].carLife < 0 && !isGameOver)
			{
				GameOver();	
			}

			// wont check for disabled or destroyed cars
			if( !activeBots[i].isBotArrested && activeBots[i].carLife > 0)// &&
			{
				isGameWon = false;
